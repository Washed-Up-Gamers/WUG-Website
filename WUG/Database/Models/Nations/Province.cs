...
public class Province
{
    ...
    [NotMapped]
    public string MapColor => District?.Color ?? "ffffff";
    ...
    public List<(string modifiername, double value)> GetStaticModifiersOfTypes(List<ProvinceModifierType?>? provincetypes, List<NationModifierType?> districttypes, bool AlsoUseDistrictModifiers, bool UseProvinceModifiers = true, bool IncludeDevStage = false)
    {
            ...
            foreach (var node in modifier.BaseStaticModifiersObj.ModifierNodes)
            {
                if ((provincetypes.Contains(node.provinceModifierType) && node.provinceModifierType is not null) || (districttypes.Contains(node.districtModifierType) && node.districtModifierType is not null))
                {
                    (string modifiername, double value) item = new()
                    {
                        ...
                        value = (double)node.GetValue(new(District, this, null, (node.provinceModifierType is not null ? ScriptScopeType.Province : ScriptScopeType.Nation)))
                    };
    ...
    public List<(string modifiername, double value)> GetStaticModifiersOfType(ProvinceModifierType? provincetype, NationModifierType? districttype, bool AlsoUseDistrictModifiers, bool UseProvinceModifiers = true, bool IncludeDevStage = false) {
        var result = new List<(string modifiername, double value)>();
        var modifiers = new List<StaticModifier>();
        if (UseProvinceModifiers)
            modifiers.AddRange(StaticModifiers);
    ...
    public double GetOverpopulationModifier()
    {
        ...
        exponent += District.GetModifierValue(NationModifierType.OverPopulationModifierExponent);
    ...
    public async ValueTask<(double growthrate, List<ProvinceConsumerGoodsData> ConsumerGoodsData)> GetMonthlyPopulationGrowth(bool UseResources = false)
    {
        ...
        BirthRate += District.GetModifierValue(NationModifierType.MonthlyBirthRate);
        BirthRate *= District.GetModifierValue(NationModifierType.MonthlyBirthRateFactor) + 1;
        ...
        DeathRate += District.GetModifierValue(NationModifierType.MonthlyDeathRate);
        DeathRate *= District.GetModifierValue(NationModifierType.MonthlyDeathRateFactor) + 1;
        ...
        PopulationGrowth *= District.GetModifierValue(NationModifierType.PopulationGrowthSpeedFactor) + 1;
    ...
    public async ValueTask HourlyTick()
    {
        ...
        buildingslots_exponent += District.GetModifierValue(NationModifierType.BuildingSlotsExponent);
        ...
        // district level
        slots *= 1 + District.GetModifierValue(NationModifierType.BuildingSlotsFactor);
